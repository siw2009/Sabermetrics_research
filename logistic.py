...
def err_entropy(realdata: int, prediction: float) -> float:
    return -realdata * ln(prediction)  if prediction > 0 else  0
...
if __name__ == '__main__':
    ...
    with open(SAVEFILEPATH, 'r') as file:
        ...
        datalen = len(data)
        for row in data:
            error = err(row[6], predict(row[7:11], weight, bias))
    ...
    logit_pi = sum(list(zip(*data_split[2]))[0]) / len(data_split[2])
    baseline = err(logit_pi, logit_pi)
    skill = (baseline - rlt) / baseline
